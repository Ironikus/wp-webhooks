<?php
if ( ! class_exists( 'WP_Webhooks_Action_delete_post' ) ) :
	 ...
	 */
	class WP_Webhooks_Action_delete_post {
		...
		public function add_action_details( $actions ){
			...
			$actions['delete_post'] = $this->action_delete_post_content();
		...
		public function add_action_callback( $response, $action, $response_ident_value, $response_api_key ){
			...
			switch( $action ){
				case 'delete_post':
					$response = $this->action_delete_post();
					break;
			}
