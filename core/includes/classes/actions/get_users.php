<?php
if ( ! class_exists( 'WP_Webhooks_Action_get_users' ) ) :
	 ...
	 */
	class WP_Webhooks_Action_get_users {
		...
		public function add_action_details( $actions ){
			...
			$actions['get_users'] = $this->action_get_users_content();
		...
		public function add_action_callback( $response, $action, $response_ident_value, $response_api_key ){
			...
			switch( $action ){
				case 'get_users':
					$response = $this->action_get_users();
					break;
			}
