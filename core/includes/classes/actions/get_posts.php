<?php
if ( ! class_exists( 'WP_Webhooks_Action_get_posts' ) ) :
	 ...
	 */
	class WP_Webhooks_Action_get_posts {
		...
		public function add_action_details( $actions ){
			...
			$actions['get_posts'] = $this->action_get_posts_content();
		...
		public function add_action_callback( $response, $action, $response_ident_value, $response_api_key ){
			...
			switch( $action ){
				case 'get_posts':
					$response = $this->action_get_posts();
					break;
			}
