<?php
if ( ! class_exists( 'WP_Webhooks_Action_delete_user' ) ) :
	 ...
	 */
	class WP_Webhooks_Action_delete_user {
		...
		public function add_action_details( $actions ){
			...
			$actions['delete_user'] = $this->action_delete_user_content();
		...
		public function add_action_callback( $response, $action, $response_ident_value, $response_api_key ){
			...
			switch( $action ){
				case 'delete_user':
					$response = $this->action_delete_user();
					break;
			}
